...
const StepNode = ({ id }: { id: string }) => (
  <div className="absolute left-1/2 -translate-x-1/2 -top-10 z-20">
    <div className="w-12 h-12 rounded-full border border-white/25 bg-black/60 backdrop-blur-sm text-white font-bold grid place-items-center shadow">
      {id}
    </div>
  </div>
);
  ...
  const steps = [
    {
      ...
      visual: (
        <FloatingPills
          pills={[
            { text: "Coaches", x: 16, y: 20 },
            { text: "Personal Brand", x: 160, y: 36 },
            { text: "E-Commerce", x: 32, y: 96 },
            { text: "Fashion", x: 176, y: 126 },
          ]}
        />
      ),
      side: "right",
    ...
    },
    {
      ...
      desc:
        "We analyze other thumbnails in your niche and replicate the best-performing results.",
      visual: <ThumbnailMockup />,
      side: "left",
    },
    {
      id: "05",
      ...
      desc:
        "All that's left is to post the video and start counting the leads that come in.",
    ...
    <div className="min-h-screen bg-black">
      ...
      <section className="py-24 relative overflow-hidden bg-black">
        ...
        <div className="max-w-7xl mx-auto px-7 sm:px-6 lg:px-8  text-center">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-light mb-9 pt-20 bg-gradient-to-r from-[#A1BFFF] via-white to-[#A649D2] bg-clip-text text-transparent drop-shadow-lg filter drop-shadow-[0_0_20px_rgba(161,191,255,0.8)] drop-shadow-[0_0_40px_rgba(166,73,210,0.6)]">
              Our Services
            </h1>
      ...
      <section ref={sectionRef} className="relative overflow-hidden py-24">
        ...
        <div className="max-w-6xl mx-auto px-4 relative">
          ...
          <div className="pointer-events-none absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-px">
            ...
            {/* Traveling light */}
            <motion.div
              className="absolute w-1 h-8 bg-gradient-to-b from-[#A1BFFF] via-white to-[#A649D2] rounded-full"
              style={{
                y: useTransform(scrollYProgress, [0, 1], [0, sectionHeight - 100])
              }}
            />
          </div>
            ...
            {steps.map((s) => (
              <div key={s.id} className="relative">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  {/* decide which side has text vs visual */}
                  {s.side === "left" ? (
                    <>
                      {/* text left */}
                      <Reveal delay={0.05}>
                        <div className="max-w-md ml-auto">
                          <Tag>{s.tag}</Tag>
                          <h3 className="text-2xl md:text-3xl lg:text-4xl font-light bg-gradient-to-r from-[#A1BFFF] via-white to-[#A649D2] bg-clip-text text-transparent mt-4">
                            {s.title}
                          </h3>
                          <p className="text-thin mt-4 leading-relaxed">
                            {s.desc}
                          </p>
                        </div>
                      </Reveal>
                      {/* visual right */}
                      <div className="mr-auto">{s.visual}</div>
                    </>
                  ) : (
                    <>
                      {/* visual left */}
                      <div className="ml-auto">{s.visual}</div>
                      {/* text right */}
                      <Reveal delay={0.05}>
